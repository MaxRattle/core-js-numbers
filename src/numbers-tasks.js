...
function getFibonacciNumber(index) {
  ...
  for (let i = 2; i <= index; i++) {
    const prevFirstNum = result[i - 1];
    const prevLastNum = result[i - 2];
    result.push(prevFirstNum + prevLastNum);
  }
  return result[index];
}
...
//   const sum = digits.reduce((accumulatedSum, digit) => accumulatedSum + Number(digit), 0);
//   return sum;
// }
...
function getFloatOnString(str) {
  ...
  // return typeof str === 'string' ? parseFloat(str) : parseFloat(Number(str));
...
function getHypotenuse(a, b) {
  // const squareHypotenuse = a ** 2 + b ** 2;
  // const sqrtHypotenuse = Math.sqrt(squareHypotenuse);
  // return sqrtHypotenuse;
